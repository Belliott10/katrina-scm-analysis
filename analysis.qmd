...
Table 1: Model Fit Diagnostics \dotfill \pageref{tbl-diagnostics}

\vspace{2em}

\section*{List of Figures}

Figure 1: Naive Comparison \dotfill \pageref{fig-naive}

Figure 2: Pre-Treatment Fit Validation \dotfill \pageref{fig-pretreatment}

Figure 3: Observed vs Synthetic Control \dotfill \pageref{fig-observed}

Figure 4: Estimated Treatment Effect \dotfill \pageref{fig-treatment}

Figure 5: Observed vs Synthetic Employment Levels \dotfill \pageref{fig-employment-levels}

Figure 6: In-Time Placebo Test \dotfill \pageref{fig-placebo}

Figure 7: In-Space Placebo Test \dotfill \pageref{fig-inspace}
...
This paper contributes to disaster economics by addressing gaps in prior Hurricane Katrina research. Unlike most studies focusing on individual outcomes or per capita measures, it examines aggregate employment at the metropolitan level, providing a direct measure of labor market recovery and resilience. Using year-over-year employment growth rates avoids level-based measurement issues while capturing employment dynamics. A 15-year pre-treatment period (1991–2005) improves counterfactual credibility and reduces overfitting compared to shorter baselines, while MSA-level analysis captures localized shocks better than state-level aggregation.
...
The focal metropolitan area is the New Orleans–Metairie MSA, which was directly and severely impacted by Hurricane Katrina in August 2005. For comparison, the analysis uses 146 other U.S. metropolitan statistical areas that were not directly exposed to Hurricane Katrina's destruction. MSAs in Louisiana (other than New Orleans), Mississippi, and Alabama that experienced direct hurricane impacts are excluded to ensure that comparison units were not themselves affected by the disaster. The data spans January 1991 through December 2025, providing 176 months before the hurricane and 244 months afterward, allowing examination of immediate, medium-term, and long-run employment effects over a full 20-year recovery period.
The 146 comparison MSAs span diverse regions and economic structures, from manufacturing-oriented MSAs like Cleveland and Dayton to energy-focused MSAs like Midland, Texas, providing substantial variation in economic characteristics.
...
The synthetic control method constructs an artificial comparison unit by taking a weighted average of control units that were not affected by the treatment. Formally, suppose we observe $J+1$ units over $T$ time periods, where unit 1 is the treated unit (New Orleans) and units $j = 2, \ldots, J+1$ are potential controls (other MSAs). Let $Y_{it}$ denote the outcome for unit $i$ at time $t$, and let $T_0$ denote the time of treatment (Hurricane Katrina). 

The synthetic control is defined as:
$$\hat{Y}_{1t}^{SC} = \sum_{j=2}^{J+1} w_j Y_{jt}$$

where $w_j \geq 0$ are non-negative weights that sum to one: $\sum_{j=2}^{J+1} w_j = 1$. These weights are chosen to minimize the distance between the treated unit's pre-treatment characteristics and the weighted average of control units. The treatment effect is then estimated as:
$$\hat{\tau}_{1t} = Y_{1t} - \hat{Y}_{1t}^{SC}$$

for periods after the hurricane ($t > T_0$). The key assumption is that absent Hurricane Katrina, New Orleans would have evolved similarly to the synthetic control.
...
Several potential weaknesses warrant acknowledgment. First, other hurricanes during the post-treatment period (Rita 2005, Gustav 2008, Isaac 2012, Ida 2021) affected New Orleans but not donor MSAs. These additional shocks could confound estimates, though they were far less severe than Katrina and any bias would likely be toward zero.
...
Third, policy endogeneity arises because the estimated effects reflect both the disaster and policy responses (substantial federal assistance, levee improvements, etc.). Separating the "pure" disaster effect from policy response effects is not possible with this design. However, the combined effect may be the policy-relevant estimand: what happens when a major disaster occurs and the government responds with typical disaster assistance.

# Results and Discussion
## Preliminary Analysis
Before presenting the synthetic control results, Figure 1 shows a naive comparison of New Orleans employment growth against the simple average of all 146 donor MSAs. Year-over-year (YoY) employment growth captures the annual percentage change in total non-farm employment (NFE). The gray lines show individual donor trajectories, revealing substantial heterogeneity across metropolitan areas. Individual MSAs exhibit widely varying employment growth rates, with some experiencing volatile fluctuations while others show more stable patterns.The blue line represents the unweighted average of donors, which provides a poor counterfactual for New Orleans due to this heterogeneity. 

An unweighted average treats all donor MSAs as equally comparable to New Orleans, regardless of how similar their pre-treatment employment patterns actually were. Some donor MSAs experienced strong growth while others stagnated, and simple averaging fails to account for New Orleans' specific pre-treatment characteristics. This motivates the synthetic control approach, which constructs an optimally-weighted combination of donors that closely matches New Orleans' pre-Katrina employment patterns. By selecting donors with similar pre-treatment trajectories and assigning them optimal weights, the synthetic control method produces a more credible counterfactual than simple averaging.
...
donor_data = df[df["MSA Title"] != treated_unit]
donors = donor_data["MSA Title"].unique()

fig, ax = plt.subplots(figsize=(10, 6))

# Plot individual donors (gray lines)
for donor in donors:
    donor_df = donor_data[donor_data["MSA Title"] == donor].sort_values("date")
    ax.plot(donor_df["date"], donor_df["YoY_NFE"], color="lightgray", linewidth=0.5, alpha=0.5)

# Calculate and plot donor pool mean (blue line)
donor_mean = donor_data.groupby("date")["YoY_NFE"].mean().reset_index()
ax.plot(donor_mean["date"], donor_mean["YoY_NFE"], color="blue", linewidth=2.5, label="Donor Pool Mean")

# Plot New Orleans (black line)
nola_df = df[df["MSA Title"] == treated_unit].sort_values("date")
ax.plot(nola_df["date"], nola_df["YoY_NFE"], color="black", linewidth=2.5, label="New Orleans-Metairie, LA MSA")

# Add vertical line for Katrina
ax.axvline(x=pd.to_datetime("2005-08-01"), color="gray", linestyle="--", linewidth=2, label="Hurricane Katrina (Aug 2005)")

# Format axes
ax.set_xlabel("Date", fontsize=11)
ax.set_ylabel("Year-over-Year Non-Farm Employment Growth (%)", fontsize=11)
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
ax.xaxis.set_major_locator(mdates.YearLocator(5))
ax.set_title("Naive Comparison - New Orleans vs Donor Pool Average)", fontsize=14, fontweight='bold')
...
Figure 2 displays the gap between actual New Orleans and synthetic control during 1991-2005, fluctuating tightly around zero throughout this extended window. This close pre-treatment match validates the identification assumption that the synthetic control provides a credible counterfactual for New Orleans.
...
Figure 3 presents observed versus synthetic control employment growth over the full study period. During the pre-treatment period through August 2005, both series track closely with minimal divergence. At the hurricane timing, there is immediate catastrophic separation between the two lines.
...
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
plt.title("Causal Impact: Observed vs Synthetic Control", fontsize=16, weight="bold")
plt.tight_layout()
plt.show()
```
\footnotesize
...
Figure 4 shows the treatment effect as the gap between observed and synthetic New Orleans employment growth. The immediate impact in late 2005 was severe, with employment growth collapsing to approximately -33% year-over-year. This represents one of the largest employment shocks to a U.S. metropolitan area in modern history, indicating that New Orleans lost roughly one-third of its employment base compared to the previous year.
...
plt.title("Estimated Treatment Effect", fontsize=16)
plt.tight_layout()
plt.show()
```
...
Figure 5 presents total nonfarm employment in levels (thousands of workers) rather than year-over-year growth rates, providing a complementary perspective on recovery. The black line shows actual New Orleans employment, while the red line shows the synthetic control.

```{python}
#| label: fig-employment-levels
...
plt.title("Observed vs Synthetic Control Employment Rate", fontsize=16)
plt.tight_layout()
plt.show()
```
...
*Note: Total nonfarm employment (thousands). Black line shows observed New Orleans; red line shows synthetic control. Employment fell from 575,000 pre-Katrina to 365,000, recovered to 480,000 by 2025. Counterfactual employment would have been approximately 650,000, indicating a persistent 26% deficit.*
...
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
plt.title("In-Time Placebo Test: Causal Impact on Year-over-Year_Non Farm Emploment", fontsize=16, weight="bold")
plt.tight_layout()
plt.show()
```
...
Figure 7 presents the spaghetti plot showing treatment effect trajectories over time. The gray lines show 30 placebo MSAs, while the thick black line shows New Orleans. The placebo gaps oscillate around zero throughout the period with no systematic pattern. No placebo unit exhibits a trajectory remotely similar to New Orleans. In contrast, New Orleans shows a clear catastrophic drop at the August 2005 treatment timing, declining to approximately -30 percentage points, followed by gradual recovery.

![In-Space Placebo Test](In_Space_Placebo_SPAGHETTI_PLOT.png){width=100% #fig-inspace}

\footnotesize

*Note: In-space placebo test. Gray lines show 30 placebo MSAs; black line shows New Orleans. Placebo effects oscillate around zero while New Orleans exhibits a distinct catastrophic drop and gradual recovery. Permutation p-value = 0.16.*
...
Several limitations deserve consideration. First, the distinction between year-over-year growth rates and absolute employment levels is crucial for interpretation. Recovery of growth rates to zero does not necessarily mean full recovery of pre-disaster employment levels if the base was permanently depressed. Second, the 2008-2009 Great Recession and 2020-2021 COVID-19 pandemic occurred during the post-treatment period, potentially confounding Katrina effects, though the synthetic control should account for common shocks and the analysis shows parallel movements during both crises. Third, my aggregate analysis masks important heterogeneity across industries and demographics, with prior literature documenting that lower-income workers and minority populations faced greater barriers to return.
...
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
plt.title("Causal Impact on Year-over-Year_Non Farm Employment", fontsize=16, weight="bold")
plt.tight_layout()
plt.show()
```
...
## Python script generating the temployment levels figure
