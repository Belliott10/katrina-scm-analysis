...
The synthetic control method constructs an artificial comparison unit by taking a weighted average of control units that were not affected by the treatment. These control units are commonly referred to as the **donor pool** in the synthetic control literature. 
Formally, suppose we observe $J+1$ units over $T$ time periods, where unit 1 is the treated unit (New Orleans) and units $j = 2, \ldots, J+1$ are potential controls (other MSAs). Let $Y_{it}$ denote the outcome for unit $i$ at time $t$, and let $T_0$ denote the time of treatment (Hurricane Katrina). 
